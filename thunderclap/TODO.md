...
- [ ] **Listen to the library in a DAW and correct by ear.** Every constant
  ...
  the N-wave base length (`kNwaveBaseSec`), the echo loop gain, and the
  balance of rumble against shocks.
...
- [ ] **Ground reflection.** Each shock arrives twice, direct and off the
  ...
  on the near channel and is not modelled.
...
- [ ] The project is not a git repository yet. `.gitignore` is in place.
