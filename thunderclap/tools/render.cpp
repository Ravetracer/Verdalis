// Minimal CLAP host used to verify ThunderClap outside a DAW: it walks the preset
...
//
//   thunderclap-render --list
//   thunderclap-render --preset close_strike --out thunder.wav --tail 15
//   thunderclap-render --all --outdir /tmp/thunder
//   thunderclap-render --selftest
...
void setupIndexer() {
   ...
   gIndexer.iface.name = "thunderclap-render";
   gIndexer.iface.vendor = "ThunderClap";
...
int main(int argc, char **argv) {
   std::string pluginPath = "./ThunderClap.clap";
   std::string presetSel, outPath = "thunder.wav", outDir = ".";
   // A thunder plays itself out after the note, so the tail is where it lives.
   double seconds = 1.0, tail = 14.0;
   int16_t key = 60;
   double velocity = 0.9;
...
int runSelfTest(const clap_plugin_entry_t *entry, double sampleRate) {
   ...
   if (state) {
      ...
      const std::string junk = "not a thunderclap state blob at all";
      ReadCtx rc2{&junk, 0};
   ...
   // --- a fixed Random Seed has to mean the same thunder every time, whatever
   // the engine happened to render before.
   {
   ...
   // preset quietly changes the sound it was saved from.
   {
      ...
      check(text.find("mode = ") != std::string::npos &&
               text.find("mode = 1") == std::string::npos,
      ...
      const std::string path = userPresetPath("My Thunder / 2 **");
      check(path.empty() || path.find("My_Thunder_2.") != std::string::npos,
      ...
      const std::string tmpdir =
         (std::filesystem::temp_directory_path() /
          ("thunderclap-selftest-" + std::to_string(
#if defined(_WIN32)
