// Opens ThunderClap's editor in a plain X11 window so the interface can be driven
...
//
//   ./thunderclap-guihost [plugin.clap] [preset.thunderclap] [seconds] [--note]
//
// It processes audio on a timer thread as well, because the editor reads its
// parameter values back through the same path a host would. With --note it
// plays one note at the start, so the bolt and the activity meter can be seen.
...
int main(int argc, char **argv) {
   ...
   host.name = "thunderclap-guihost";
   host.vendor = "ThunderClap";
   host.url = "";
   host.version = "1.0";
   ...
   wc.lpszClassName = L"ThunderClapGuiHost";
   RegisterClassExW(&wc);

   RECT wanted{0, 0, static_cast<LONG>(w), static_cast<LONG>(h)};
   AdjustWindowRect(&wanted, WS_OVERLAPPEDWINDOW, FALSE);
   HWND hostWindow = CreateWindowExW(0, L"ThunderClapGuiHost", L"ThunderClap", WS_OVERLAPPEDWINDOW,
   ...
   XStoreName(dpy, win, "ThunderClap");
   ...
   std::thread audio([&] {
      ...
      // One note-on in the first block, if asked for.
      static clap_event_note_t noteOn{};
      ...
      noteOn.key = 60;
      noteOn.velocity = 1.0;
      clap_input_events_t inNote{};
      inNote.size = [](const clap_input_events_t *) -> uint32_t { return 1; };
      inNote.get = [](const clap_input_events_t *, uint32_t) -> const clap_event_header_t * {
         return &noteOn.header;
      };
      ...
      while (gRunning.load()) {
         ...
         pr.in_events = (first && playNote) ? &inNote : &in;
         pr.out_events = &out;
         first = false;
