...
constexpr uint32_t kStateMagic = 0x504C4354u; // 'TCLP' little-endian
constexpr uint32_t kStateVersion = 1;

} // namespace

#ifdef THUNDERCLAP_WITH_GUI
class ThunderClapPlugin final : public GuiDelegate {
#else
class ThunderClapPlugin {
#endif
public:
   explicit ThunderClapPlugin(const clap_host_t *host) : mHost(host) {
...
private:
   static ThunderClapPlugin *self(const clap_plugin_t *p) {
      return static_cast<ThunderClapPlugin *>(p->plugin_data);
   }
   ...
   static bool audioPortsGet(const clap_plugin_t *, uint32_t index, bool isInput,
      ...
      std::memset(info, 0, sizeof(*info));
      info->id = 0;
      std::snprintf(info->name, sizeof(info->name), "Thunder Out");
   ...
   static bool voiceInfoGet(const clap_plugin_t *, clap_voice_info_t *info) {
      info->voice_count = ThunderEngine::kMaxVoices;
      info->voice_capacity = ThunderEngine::kMaxVoices;
   ...
   clap_process_status process(const clap_process_t *pr) {
      ...
      while (frame < numFrames) {
         // Anything the main thread changed -- a preset, a knob, a loaded state
         // -- reaches the engine before this block's notes do. A note captures
         // the mode and envelope it starts with, so a note in the same block as
         // a preset load has to see the preset, not the values before it.
         ...
         // And again after the events, which may themselves have been
         // parameter changes.
         if (mParamsDirty.exchange(false, std::memory_order_acq_rel))
            syncEngineParams();
      ...
      // Published for the window's activity meter and its lightning bolt; the
      // GUI never reads engine state directly.
      mShockMeter.store(mEngine.activeShockCount(), std::memory_order_relaxed);
      mFlashMeter.store(mEngine.flashCounter(), std::memory_order_relaxed);
   ...
   static bool guiCreate(const clap_plugin_t *p, const char *api, bool isFloating) {
      ...
      plug->mGui = thunderclap::createGui(*plug);
      if (!plug->mGui)
         return false;
      plug->startGuiClock();
      return true;
   ...
   std::atomic<uint32_t> mShockMeter{0};
   std::atomic<uint32_t> mFlashMeter{0};
...
const clap_plugin_t *factoryCreate(const clap_plugin_factory_t *, const clap_host_t *host,
   ...
   if (!pluginId || std::strcmp(pluginId, kPluginId) != 0)
      return nullptr;
   auto *plug = new ThunderClapPlugin(host);
...
bool thunderclapEntryInit(const char *) { return true; }
