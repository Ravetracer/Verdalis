...
// and Cairo for the drawing, so ThunderClap stays a single .clap file with no
...
public:
   ...
   // Shock waves currently sounding, and the pool they come from, published by
   // the audio thread.
   virtual uint32_t guiShockCount() const = 0;
   virtual uint32_t guiShockLimit() const = 0;
   // Counts every flash so far; the window lights a bolt when it changes.
   virtual uint32_t guiFlashCounter() const = 0;
