...
// Four per line, which is one row of the panel.
...
// the violet of a lightning channel rather than the blue of rain.
constexpr Rgb kBgTop = {0.094, 0.090, 0.125};
constexpr Rgb kBgBottom = {0.055, 0.051, 0.078};
constexpr Rgb kPanelFill = {0.129, 0.125, 0.173};
constexpr Rgb kPanelEdge = {0.184, 0.180, 0.247};
constexpr Rgb kKnobFace = {0.067, 0.063, 0.094};
constexpr Rgb kTrack = {0.208, 0.204, 0.278};
constexpr Rgb kAccent = {0.702, 0.588, 0.980};
constexpr Rgb kBolt = {0.980, 0.965, 0.900};
constexpr Rgb kText = {0.918, 0.906, 0.945};
constexpr Rgb kTextDim = {0.541, 0.518, 0.620};
constexpr Rgb kTextMute = {0.400, 0.380, 0.475};
...
// Every cell is one column wide. Filter Type prints "Bandpass" and Mode prints
// "One Shot" in a single cell, and the layout comes out exactly full.
...
public:
   ...
   static const wchar_t *windowClassName() { return L"ThunderClapWindow"; }
   ...
   bool open() override {
      ...
      mWindow = CreateWindowExW(0, windowClassName(), L"ThunderClap", WS_POPUP | WS_CLIPCHILDREN,
...
private:
   ...
   bool needsRepaint() {
      ...
      const uint32_t shocks = mDelegate.guiShockCount();
      return shocks != mLastShockCount || mMeterFill > 0.001 || mDecayFrames > 0 ||
             mBoltFrames > 0 || mDelegate.guiFlashCounter() != mLastFlashCounter;
   }
   ...
   // The header carries the wordmark and, whenever the engine lights a flash, a
   // lightning bolt that is grown from the flash's own number, so no two look
   // alike either. It lingers a few frames and fades.
   void drawHeader(cairo_t *cr) {
      const uint32_t flashes = mDelegate.guiFlashCounter();
      if (flashes != mLastFlashCounter) {
         mLastFlashCounter = flashes;
         mBoltFrames = kBoltFrames;
         mBoltSeed = flashes;
      }
      if (mBoltFrames > 0) {
         --mBoltFrames;
         const double fade = static_cast<double>(mBoltFrames) / kBoltFrames;
         cairo_save(cr);
      ...
      double advance = textWidth(cr, "Thunder", 30, true);
      setColor(cr, kAccent);
      cairo_move_to(cr, kMargin + advance + 2, baseline);
      cairo_show_text(cr, "Clap");

      setColor(cr, kTextMute);
      drawText(cr, kMargin + kContentW, 30, "SYNTHETIC THUNDER INSTRUMENT", 9, true, Align::Right);
      char ver[64];
   ...
   // A jagged line from the top of the header to its bottom edge, with a branch
   // or two, in the right-hand half where it does not run through the wordmark.
   // Deterministic in the flash number, so the same flash draws the same bolt
   // on every frame while it fades.
   void drawBolt(cairo_t *cr, double fade) {
      uint32_t h = mBoltSeed * 2654435761u + 0x9E3779B9u;
      auto next = [&h]() {
         h ^= h << 13;
         h ^= h >> 17;
         h ^= h << 5;
      ...
      const double x0 = kMargin + kContentW * (0.45 + 0.45 * next());
      ...
      auto strand = [&](double x, double y, double yEnd, double drift, double width, double alpha) {
         ...
         setColor(cr, kAccent, alpha * glowA);
         cairo_stroke_preserve(cr);
         cairo_set_line_width(cr, width);
         setColor(cr, kBolt, alpha * coreA);
         cairo_stroke(cr);
      };

      cairo_set_line_join(cr, CAIRO_LINE_JOIN_MITER);
      cairo_set_line_cap(cr, CAIRO_LINE_CAP_ROUND);
      strand(x0, -4.0, kHeaderH + 4.0, (next() - 0.5) * 3.0, 1.6, 1.0);
      const int branches = 1 + static_cast<int>(next() * 2.0);
      for (int b = 0; b < branches; ++b) {
         const double by = 8.0 + next() * (kHeaderH * 0.5);
   ...
   void drawMeter(cairo_t *cr) {
      ...
      const uint32_t shocks = mDelegate.guiShockCount();
      const uint32_t limit = std::max<uint32_t>(1, mDelegate.guiShockLimit());
      mLastShockCount = shocks;
      // Compressed rather than linear: a rumble that has a few dozen shocks in
      // the air out of a pool of thousands is a perfectly ordinary thunder, and
      // a linear meter would show nothing at all for it.
      const double load = std::min(1.0, static_cast<double>(shocks) / static_cast<double>(limit));
      ...
      // The right-hand strip of the panel is the output meter; the shock
      ...
      char info[64];
      std::snprintf(info, sizeof(info), "%u shocks", shocks);
      setColor(cr, kTextDim);
      drawText(cr, gx, mMeter.y + mMeter.h - 10, info, 9, false, Align::Left);
      std::snprintf(info, sizeof(info), "%u flashes", mLastFlashCounter);
   ...
   // nothing about a distant rumble.
   void drawOutputMeter(cairo_t *cr, double x, double y, double w, double h) {
      ...
      for (int ch = 0; ch < 2; ++ch) {
         ...
         if (fh > 0.5) {
            ...
            // work for you, and a close strike gets there.
   ...
   uint32_t mLastShockCount = 0;
   uint32_t mLastFlashCounter = 0;
   uint32_t mBoltSeed = 0;
   int mBoltFrames = 0;
   static constexpr int kBoltFrames = 24;
