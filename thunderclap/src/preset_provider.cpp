// Preset discovery: tells the host where ThunderClap presets live and what is in
...
bool providerInit(const clap_preset_discovery_provider_t *p) {
   ...
   const clap_preset_discovery_filetype_t filetype = {
      "ThunderClap Preset", "ThunderClap rain synthesis preset", kPresetExtension};
   ...
   if (!self->factoryDir.empty()) {
      const clap_preset_discovery_location_t location = {
         CLAP_PRESET_DISCOVERY_IS_FACTORY_CONTENT, "ThunderClap Factory Presets",
   ...
   if (!self->userDir.empty()) {
      ...
      if (std::filesystem::is_directory(self->userDir, ec)) {
         const clap_preset_discovery_location_t location = {
            CLAP_PRESET_DISCOVERY_IS_USER_CONTENT, "ThunderClap User Presets",
