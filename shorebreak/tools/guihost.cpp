// Opens ShoreBreak's editor in a plain X11 window so the interface can be driven
...
//
//   ./shorebreak-guihost [plugin.clap] [preset.shorebreak] [seconds]
//
...
int main(int argc, char **argv) {
   const std::string pluginPath = argc > 1 ? argv[1] : "./ShoreBreak.clap";
   ...
   host.name = "shorebreak-guihost";
   host.vendor = "ShoreBreak";
   host.url = "";
   host.version = "1.0";
   ...
   wc.lpszClassName = L"ShoreBreakGuiHost";
   RegisterClassExW(&wc);

   RECT wanted{0, 0, static_cast<LONG>(w), static_cast<LONG>(h)};
   AdjustWindowRect(&wanted, WS_OVERLAPPEDWINDOW, FALSE);
   HWND hostWindow = CreateWindowExW(0, L"ShoreBreakGuiHost", L"ShoreBreak", WS_OVERLAPPEDWINDOW,
