// Minimal CLAP host used to verify ShoreBreak outside a DAW: it walks the preset
...
//
//   shorebreak-render --list
//   shorebreak-render --preset steady_rain --out rain.wav --seconds 8
//   shorebreak-render --all --outdir /tmp/rain
//   shorebreak-render --selftest
...
void setupIndexer() {
   ...
   gIndexer.iface.name = "shorebreak-render";
   gIndexer.iface.vendor = "ShoreBreak";
...
int main(int argc, char **argv) {
   std::string pluginPath = "./ShoreBreak.clap";
...
int runSelfTest(const clap_plugin_entry_t *entry, double sampleRate) {
   ...
   if (state) {
      ...
      const std::string junk = "not a shorebreak state blob at all";
      ReadCtx rc2{&junk, 0};
   ...
   // preset quietly changes the sound it was saved from.
   {
      ...
      check(text.find("shore_type = ") != std::string::npos &&
               text.find("shore_type = 0") == std::string::npos,
      ...
      const std::string tmpdir =
         (std::filesystem::temp_directory_path() /
          ("shorebreak-selftest-" + std::to_string(
#if defined(_WIN32)
