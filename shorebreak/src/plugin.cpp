...
#ifdef SHOREBREAK_WITH_GUI
class ShoreBreakPlugin final : public GuiDelegate {
#else
class ShoreBreakPlugin {
#endif
public:
   explicit ShoreBreakPlugin(const clap_host_t *host) : mHost(host) {
...
private:
   static ShoreBreakPlugin *self(const clap_plugin_t *p) {
      return static_cast<ShoreBreakPlugin *>(p->plugin_data);
   }
   ...
   static bool voiceInfoGet(const clap_plugin_t *, clap_voice_info_t *info) {
      info->voice_count = WaveEngine::kMaxVoices;
      info->voice_capacity = WaveEngine::kMaxVoices;
   ...
   clap_process_status process(const clap_process_t *pr) {
      ...
      // state directly.
      mWaveMeter.store(mEngine.activeWaveCount(), std::memory_order_relaxed);
   ...
   uint32_t guiVoiceCount() const override {
      return mWaveMeter.load(std::memory_order_relaxed);
   }

   uint32_t guiVoiceLimit() const override {
      return static_cast<uint32_t>(realValue(kParamMaxWaves));
   }
   ...
   static bool guiCreate(const clap_plugin_t *p, const char *api, bool isFloating) {
      ...
      plug->mGui = shorebreak::createGui(*plug);
      if (!plug->mGui)
         return false;
      plug->startGuiClock();
      return true;
   ...
   std::atomic<uint32_t> mWaveMeter{0};
...
const clap_plugin_t *factoryCreate(const clap_plugin_factory_t *, const clap_host_t *host,
   ...
   if (!pluginId || std::strcmp(pluginId, kPluginId) != 0)
      return nullptr;
   auto *plug = new ShoreBreakPlugin(host);
...
bool shorebreakEntryInit(const char *) { return true; }
void shorebreakEntryDeinit() {}

const void *shorebreakEntryGetFactory(const char *factoryId) {
   if (!factoryId)
      return nullptr;
   if (std::strcmp(factoryId, CLAP_PLUGIN_FACTORY_ID) == 0)
      return &shorebreak::gPluginFactory;
   if (std::strcmp(factoryId, CLAP_PRESET_DISCOVERY_FACTORY_ID) == 0 ||
       std::strcmp(factoryId, CLAP_PRESET_DISCOVERY_FACTORY_ID_COMPAT) == 0)
      return shorebreak::presetDiscoveryFactory();
   return nullptr;
}
}
