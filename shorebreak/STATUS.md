...
A whole folder exports as one **preset pack** (`.shorebreakpack`), written to a
...
ShoreBreak ships as a VST3 alongside the CLAP, for Linux and Windows, in every
release archive. It is not a port: the clap-wrapper hosts the same
`ShoreBreak-impl` static library the `.clap` is built from, so the engine, the
