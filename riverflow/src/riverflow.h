...
constexpr char kPluginId[] = "de.ravetracer.riverflow";
constexpr char kPluginName[] = "RiverFlow";
...
#ifdef RIVERFLOW_CMAKE_VERSION
constexpr bool sameString(const char *a, const char *b) {
   return *a == *b && (*a == '\0' || sameString(a + 1, b + 1));
}
static_assert(sameString(kPluginVersion, RIVERFLOW_CMAKE_VERSION),
...
constexpr char kPluginDescription[] =
   "Fully synthetic running water. No samples: from a river's roar down to one drop "
   "on a stone, every part of it is computed while it plays.";

constexpr char kPresetExtension[] = "riverflow";
constexpr char kProviderId[] = "de.ravetracer.riverflow.preset-provider";
