...
#ifdef RIVERFLOW_WITH_GUI
class RiverFlowPlugin final : public GuiDelegate {
#else
class RiverFlowPlugin {
#endif
public:
   explicit RiverFlowPlugin(const clap_host_t *host) : mHost(host) {
...
private:
   static RiverFlowPlugin *self(const clap_plugin_t *p) {
      return static_cast<RiverFlowPlugin *>(p->plugin_data);
   }
   ...
   static bool voiceInfoGet(const clap_plugin_t *, clap_voice_info_t *info) {
      info->voice_count = RiverEngine::kMaxVoices;
      info->voice_capacity = RiverEngine::kMaxVoices;
   ...
   clap_process_status process(const clap_process_t *pr) {
      ...
      // state directly.
      mPocketMeter.store(mEngine.activePocketCount(), std::memory_order_relaxed);
      mDabbleMeter.store(mEngine.dabbleCounter(), std::memory_order_relaxed);
   ...
   uint32_t guiVoiceCount() const override {
      return mPocketMeter.load(std::memory_order_relaxed);
   }

   uint32_t guiVoiceLimit() const override {
      return static_cast<uint32_t>(realValue(kParamMaxEvents));
   }

   uint32_t guiEventCounter() const override {
      return mDabbleMeter.load(std::memory_order_relaxed);
   }
   ...
   static bool guiCreate(const clap_plugin_t *p, const char *api, bool isFloating) {
      ...
      plug->mGui = riverflow::createGui(*plug);
      if (!plug->mGui)
         return false;
      plug->startGuiClock();
      return true;
   ...
   std::atomic<uint32_t> mPocketMeter{0};
   std::atomic<uint32_t> mDabbleMeter{0};
...
const clap_plugin_t *factoryCreate(const clap_plugin_factory_t *, const clap_host_t *host,
   ...
   if (!pluginId || std::strcmp(pluginId, kPluginId) != 0)
      return nullptr;
   auto *plug = new RiverFlowPlugin(host);
...
bool riverflowEntryInit(const char *) { return true; }
