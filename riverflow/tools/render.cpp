// Minimal CLAP host used to verify RiverFlow outside a DAW: it walks the preset
...
//
//   riverflow-render --list
//   riverflow-render --preset mountain_river --out river.wav --seconds 8
//   riverflow-render --all --outdir /tmp/river
//   riverflow-render --selftest
...
void setupIndexer() {
   ...
   gIndexer.iface.name = "riverflow-render";
   gIndexer.iface.vendor = "RiverFlow";
...
int main(int argc, char **argv) {
   std::string pluginPath = "./RiverFlow.clap";
...
int runSelfTest(const clap_plugin_entry_t *entry, double sampleRate) {
   ...
   if (state) {
      ...
      const std::string junk = "not a riverflow state blob at all";
      ReadCtx rc2{&junk, 0};
   ...
   // preset quietly changes the sound it was saved from.
   {
      ...
      check(text.find("water_type = ") != std::string::npos &&
               text.find("water_type = 0") == std::string::npos,
      ...
      const std::string tmpdir =
         (std::filesystem::temp_directory_path() /
          ("riverflow-selftest-" + std::to_string(
#if defined(_WIN32)
