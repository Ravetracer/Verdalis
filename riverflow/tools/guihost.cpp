// Opens RiverFlow's editor in a plain X11 window so the interface can be driven
...
//
//   ./riverflow-guihost [plugin.clap] [preset.riverflow] [seconds]
//
...
int main(int argc, char **argv) {
   const std::string pluginPath = argc > 1 ? argv[1] : "./RiverFlow.clap";
   ...
   host.name = "riverflow-guihost";
   host.vendor = "RiverFlow";
   host.url = "";
   host.version = "1.0";
   ...
   wc.lpszClassName = L"RiverFlowGuiHost";
   RegisterClassExW(&wc);

   RECT wanted{0, 0, static_cast<LONG>(w), static_cast<LONG>(h)};
   AdjustWindowRect(&wanted, WS_OVERLAPPEDWINDOW, FALSE);
   HWND hostWindow = CreateWindowExW(0, L"RiverFlowGuiHost", L"RiverFlow", WS_OVERLAPPEDWINDOW,
