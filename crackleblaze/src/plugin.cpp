...
#ifdef CRACKLEBLAZE_WITH_GUI
class CrackleBlazePlugin final : public GuiDelegate {
#else
class CrackleBlazePlugin {
#endif
public:
   explicit CrackleBlazePlugin(const clap_host_t *host) : mHost(host) {
...
private:
   static CrackleBlazePlugin *self(const clap_plugin_t *p) {
      return static_cast<CrackleBlazePlugin *>(p->plugin_data);
   }
   ...
   static void paramsFlush(const clap_plugin_t *p, const clap_input_events_t *in,
      ...
      CrackleBlazePlugin *plug = self(p);
   ...
   static bool voiceInfoGet(const clap_plugin_t *, clap_voice_info_t *info) {
      info->voice_count = FireEngine::kMaxVoices;
      info->voice_capacity = FireEngine::kMaxVoices;
   ...
   clap_process_status process(const clap_process_t *pr) {
      ...
      mCrackleMeter.store(mEngine.crackleCounter(), std::memory_order_relaxed);
   ...
   uint32_t guiEventCounter() const override {
      return mCrackleMeter.load(std::memory_order_relaxed);
   }
   ...
   static bool guiCreate(const clap_plugin_t *p, const char *api, bool isFloating) {
      ...
      plug->mGui = crackleblaze::createGui(*plug);
      if (!plug->mGui)
         return false;
      plug->startGuiClock();
      return true;
   ...
   std::atomic<uint32_t> mCrackleMeter{0};
...
const clap_plugin_t *factoryCreate(const clap_plugin_factory_t *, const clap_host_t *host,
   ...
   if (!pluginId || std::strcmp(pluginId, kPluginId) != 0)
      return nullptr;
   auto *plug = new CrackleBlazePlugin(host);
...
const void *crackleblazeEntryGetFactory(const char *factoryId) {
   ...
   if (std::strcmp(factoryId, CLAP_PLUGIN_FACTORY_ID) == 0)
      return &crackleblaze::gPluginFactory;
