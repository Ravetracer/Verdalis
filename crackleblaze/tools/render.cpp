// Minimal CLAP host used to verify CrackleBlaze outside a DAW: it walks the preset
...
//
//   crackleblaze-render --list
//   crackleblaze-render --preset camp_fire --out fire.wav --seconds 8
//   crackleblaze-render --all --outdir /tmp/fire
//   crackleblaze-render --selftest
...
void setupIndexer() {
   ...
   gIndexer.iface.name = "crackleblaze-render";
   gIndexer.iface.vendor = "CrackleBlaze";
...
int main(int argc, char **argv) {
   std::string pluginPath = "./CrackleBlaze.clap";
...
int runSelfTest(const clap_plugin_entry_t *entry, double sampleRate) {
   ...
   if (state) {
      ...
      const std::string junk = "not a crackleblaze state blob at all";
      ReadCtx rc2{&junk, 0};
   ...
   // preset quietly changes the sound it was saved from.
   {
      ...
      check(text.find("fire_type = ") != std::string::npos &&
               text.find("fire_type = 0") == std::string::npos,
      ...
      const std::string tmpdir =
         (std::filesystem::temp_directory_path() /
          ("crackleblaze-selftest-" + std::to_string(
#if defined(_WIN32)
