// Opens CrackleBlaze's editor in a plain X11 window so the interface can be driven
...
//
//   ./crackleblaze-guihost [plugin.clap] [preset.crackleblaze] [seconds]
//
...
int main(int argc, char **argv) {
   const std::string pluginPath = argc > 1 ? argv[1] : "./CrackleBlaze.clap";
   ...
   host.name = "crackleblaze-guihost";
   host.vendor = "CrackleBlaze";
   host.url = "";
   host.version = "1.0";
   ...
   wc.lpszClassName = L"CrackleBlazeGuiHost";
   RegisterClassExW(&wc);

   RECT wanted{0, 0, static_cast<LONG>(w), static_cast<LONG>(h)};
   AdjustWindowRect(&wanted, WS_OVERLAPPEDWINDOW, FALSE);
   HWND hostWindow = CreateWindowExW(0, L"CrackleBlazeGuiHost", L"CrackleBlaze", WS_OVERLAPPEDWINDOW,
   ...
   XStoreName(dpy, win, "CrackleBlaze");
