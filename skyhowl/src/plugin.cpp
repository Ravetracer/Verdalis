...
constexpr uint32_t kStateMagic = 0x4C574B53u; // 'SKWL' little-endian
constexpr uint32_t kStateVersion = 1;

} // namespace

#ifdef SKYHOWL_WITH_GUI
class SkyHowlPlugin final : public GuiDelegate {
#else
class SkyHowlPlugin {
#endif
public:
   explicit SkyHowlPlugin(const clap_host_t *host) : mHost(host) {
...
private:
   static SkyHowlPlugin *self(const clap_plugin_t *p) {
      return static_cast<SkyHowlPlugin *>(p->plugin_data);
   }
   ...
   static bool audioPortsGet(const clap_plugin_t *, uint32_t index, bool isInput,
      ...
      std::memset(info, 0, sizeof(*info));
      info->id = 0;
      std::snprintf(info->name, sizeof(info->name), "Wind Out");
   ...
   static bool voiceInfoGet(const clap_plugin_t *, clap_voice_info_t *info) {
      info->voice_count = WindEngine::kMaxVoices;
      info->voice_capacity = WindEngine::kMaxVoices;
   ...
   clap_process_status process(const clap_process_t *pr) {
      ...
      uint32_t vents = mVentRequests.exchange(0, std::memory_order_acquire);
      if (vents > kMaxVentsPerBlock)
         vents = kMaxVentsPerBlock;
      for (uint32_t i = 0; i < vents; ++i)
         mEngine.triggerVent();
      ...
      // state directly.
      mGustMeter.store(mEngine.activeGustCount(), std::memory_order_relaxed);
      mGustCounterMeter.store(mEngine.gustCounter(), std::memory_order_relaxed);
   ...
   uint32_t guiVoiceCount() const override {
      return mGustMeter.load(std::memory_order_relaxed);
   }

   uint32_t guiVoiceLimit() const override {
      return static_cast<uint32_t>(realValue(kParamMaxGusts));
   }

   // The header ornament leans on the wind every time a gust arrives, so it
   // needs the count rather than only the load.
   uint32_t guiEventCounter() const override {
      return mGustCounterMeter.load(std::memory_order_relaxed);
   }

   // Clicking the version label. The window runs on the main thread and must
   // not touch the engine, so the click is counted here and the audio thread
   // consumes the count on its next process(). request_process() is what makes
   // it work with no note held: the engine reports silence when nothing is
   // sounding, so the host may well have stopped calling process at all.
   void guiVersionClicked() override {
      mVentRequests.fetch_add(1, std::memory_order_relaxed);
   ...
   static bool guiCreate(const clap_plugin_t *p, const char *api, bool isFloating) {
      ...
      plug->mGui = skyhowl::createGui(*plug);
      if (!plug->mGui)
         return false;
      plug->startGuiClock();
      return true;
   ...
   std::atomic<uint32_t> mGustMeter{0};
   std::atomic<uint32_t> mGustCounterMeter{0};
   // Requests posted by the editor, drained by the audio thread. Capped per
   // block so that a stuck mouse button cannot spawn thousands in one go.
   static constexpr uint32_t kMaxVentsPerBlock = 4;
   std::atomic<uint32_t> mVentRequests{0};
...
const clap_plugin_t *factoryCreate(const clap_plugin_factory_t *, const clap_host_t *host,
   ...
   if (!pluginId || std::strcmp(pluginId, kPluginId) != 0)
      return nullptr;
   auto *plug = new SkyHowlPlugin(host);
...
bool skyhowlEntryInit(const char *) { return true; }
void skyhowlEntryDeinit() {}

const void *skyhowlEntryGetFactory(const char *factoryId) {
   if (!factoryId)
      return nullptr;
   if (std::strcmp(factoryId, CLAP_PLUGIN_FACTORY_ID) == 0)
      return &skyhowl::gPluginFactory;
   if (std::strcmp(factoryId, CLAP_PRESET_DISCOVERY_FACTORY_ID) == 0 ||
       std::strcmp(factoryId, CLAP_PRESET_DISCOVERY_FACTORY_ID_COMPAT) == 0)
      return skyhowl::presetDiscoveryFactory();
   return nullptr;
}
}
