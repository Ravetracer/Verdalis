// Opens SkyHowl's editor in a plain X11 window so the interface can be driven
...
//
//   ./skyhowl-guihost [plugin.clap] [preset.skyhowl] [seconds]
//
...
int main(int argc, char **argv) {
   const std::string pluginPath = argc > 1 ? argv[1] : "./SkyHowl.clap";
   ...
   wc.lpszClassName = L"SkyHowlGuiHost";
   RegisterClassExW(&wc);

   RECT wanted{0, 0, static_cast<LONG>(w), static_cast<LONG>(h)};
   AdjustWindowRect(&wanted, WS_OVERLAPPEDWINDOW, FALSE);
   HWND hostWindow = CreateWindowExW(0, L"SkyHowlGuiHost", L"SkyHowl", WS_OVERLAPPEDWINDOW,
