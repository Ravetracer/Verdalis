// Minimal CLAP host used to verify SkyHowl outside a DAW: it walks the preset
...
//
//   skyhowl-render --list
//   skyhowl-render --preset open_plain --out wind.wav --seconds 20
//   skyhowl-render --all --outdir /tmp/wind
//   skyhowl-render --selftest
...
void setupIndexer() {
   ...
   gIndexer.iface.name = "skyhowl-render";
   gIndexer.iface.vendor = "SkyHowl";
...
int main(int argc, char **argv) {
   std::string pluginPath = "./SkyHowl.clap";
...
int runSelfTest(const clap_plugin_entry_t *entry, double sampleRate) {
   ...
   if (state) {
      ...
      const std::string junk = "not a skyhowl state blob at all";
      ReadCtx rc2{&junk, 0};
   ...
   // preset quietly changes the sound it was saved from.
   {
      ...
      check(text.find("obstacle = ") != std::string::npos &&
               text.find("obstacle = 0") == std::string::npos,
      ...
      const std::string tmpdir =
         (std::filesystem::temp_directory_path() /
          ("skyhowl-selftest-" + std::to_string(
#if defined(_WIN32)
