...
// The space the rain falls in: a room, a cave, a courtyard.
//
...
// hear -- the reference recording of one has a reflection 55 ms after each drop
...
public:
   ...
   void clear() {
      for (int i = 0; i < kLines; ++i) {
         ...
         // rain after a reset, and that includes where the tank's lines are in
         // their slow wander.
   ...
   inline void tick(float inL, float inR, float &outL, float &outR) {
      ...
      // diffused so that dense rain does not enter as a coherent flutter.
      ...
      for (int i = 0; i < kLines; ++i) {
         ...
         // accumulating anything below the audible rain.
      ...
      // Left listens to the lines the left input feeds, so a drop on one side
...
private:
   ...
   // after the drop at -7 dB and is the most audible thing about the room.
   ...
   // 0.05 dB/m, which is the damp air of somewhere it rains, times the speed
   // of sound.
   static constexpr float kAirRefHz = 6000.0f;
   static constexpr float kAirDbPerSec = 17.0f;
   static constexpr float kLoopHighpassHz = 35.0f;
