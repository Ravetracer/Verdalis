...
## Preset folders and packs (1.9.0)
...
A whole folder exports as one **preset pack** (`.rainydaypack`), written to a
...
RainyDay ships as a VST3 alongside the CLAP, for Linux and Windows, in every
release archive. It is not a port: the clap-wrapper hosts the same
`RainyDay-impl` static library the `.clap` is built from, so the engine, the
