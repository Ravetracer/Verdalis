...
constexpr uint32_t kStateMagic = 0x44594E52u; // 'RNYD' little-endian, the suite's
constexpr uint32_t kStateVersion = 1;

} // namespace

#ifdef WHOOSHPACT_WITH_GUI
class WhooshPactPlugin final : public GuiDelegate {
#else
class WhooshPactPlugin {
#endif
public:
   explicit WhooshPactPlugin(const clap_host_t *host) : mHost(host) {
...
private:
   static WhooshPactPlugin *self(const clap_plugin_t *p) {
      return static_cast<WhooshPactPlugin *>(p->plugin_data);
   }
   ...
   void syncEngineParams() {
      ...
      // gesture
      p.type = static_cast<int>(realValue(kParamType));
      p.blend = static_cast<float>(realValue(kParamBlend));
      p.spanSec = static_cast<float>(realValue(kParamSpan));
      p.peak = static_cast<float>(realValue(kParamPeak));
      p.hold = static_cast<float>(realValue(kParamHold));
      p.rise = static_cast<float>(realValue(kParamRise));
      p.fall = static_cast<float>(realValue(kParamFall));
      p.variation = static_cast<float>(realValue(kParamVariation));

      // air
      p.noise = static_cast<int>(realValue(kParamNoise));
      p.airGain = dbToGain(static_cast<float>(realValue(kParamAirLevel)));
      p.airCutoffHz = static_cast<float>(realValue(kParamAirCutoff));
      p.airSweepOct = static_cast<float>(realValue(kParamAirSweep));
      p.airReso = static_cast<float>(realValue(kParamAirReso));
      p.airCurve = static_cast<float>(realValue(kParamAirCurve));
      p.airTilt = static_cast<float>(realValue(kParamAirTilt));
      p.airWidth = static_cast<float>(realValue(kParamAirWidth));

      // tone
      p.wave = static_cast<int>(realValue(kParamWave));
      p.toneGain = dbToGain(static_cast<float>(realValue(kParamToneLevel)));
      p.tonePitchHz = static_cast<float>(realValue(kParamTonePitch));
      p.toneGlideSt = static_cast<float>(realValue(kParamToneGlide));
      p.toneDetune = static_cast<float>(realValue(kParamToneDetune));
      p.toneWidth = static_cast<float>(realValue(kParamToneWidth));

      // motion
      p.panStart = static_cast<float>(realValue(kParamPanStart));
      p.panEnd = static_cast<float>(realValue(kParamPanEnd));
      ...
      p.spaceWidth = static_cast<float>(realValue(kParamSpaceWidth));

      // sub
      p.subGain = dbToGain(static_cast<float>(realValue(kParamSubLevel)));
      p.subPitchHz = static_cast<float>(realValue(kParamSubPitch));
      p.subDropSt = static_cast<float>(realValue(kParamSubDrop));
      p.subDecaySec = static_cast<float>(realValue(kParamSubDecay)) * 0.001f;
      p.subDrive = static_cast<float>(realValue(kParamSubDrive));
      p.subClick = static_cast<float>(realValue(kParamSubClick));

      // hit
      p.hitGain = dbToGain(static_cast<float>(realValue(kParamHitLevel)));
      p.hitToneHz = static_cast<float>(realValue(kParamHitTone));
      p.hitDecaySec = static_cast<float>(realValue(kParamHitDecay)) * 0.001f;
      p.hitNoise = static_cast<float>(realValue(kParamHitNoise));
      p.hitBody = static_cast<float>(realValue(kParamHitBody));
      p.hitTime = static_cast<float>(realValue(kParamHitTime));

      // flutter
      p.flutterDepth = static_cast<float>(realValue(kParamFlutterDepth));
      p.flutterStartHz = static_cast<float>(realValue(kParamFlutterStart));
      p.flutterEndHz = static_cast<float>(realValue(kParamFlutterEnd));
      p.flutterShape = static_cast<int>(realValue(kParamFlutterShape));
      p.flutterTarget = static_cast<int>(realValue(kParamFlutterTarget));
      p.flutterSmooth = static_cast<float>(realValue(kParamFlutterSmooth));

      // eq
      p.highpassHz = static_cast<float>(realValue(kParamHighpass));
      p.lowpassHz = static_cast<float>(realValue(kParamLowpass));
      p.eqLowFreqHz = static_cast<float>(realValue(kParamEqLowFreq));
      p.eqLowGainDb = static_cast<float>(realValue(kParamEqLowGain));
      p.eqMidFreqHz = static_cast<float>(realValue(kParamEqMidFreq));
      p.eqMidGainDb = static_cast<float>(realValue(kParamEqMidGain));
      p.eqHighFreqHz = static_cast<float>(realValue(kParamEqHighFreq));
      p.eqHighGainDb = static_cast<float>(realValue(kParamEqHighGain));
      ...
      p.velToTone = static_cast<float>(realValue(kParamVelToTone));
   ...
   static bool audioPortsGet(const clap_plugin_t *, uint32_t index, bool isInput,
      ...
      std::memset(info, 0, sizeof(*info));
      info->id = 0;
      std::snprintf(info->name, sizeof(info->name), "Gesture Out");
   ...
   static bool voiceInfoGet(const clap_plugin_t *, clap_voice_info_t *info) {
      info->voice_count = GestureEngine::kMaxVoices;
      info->voice_capacity = GestureEngine::kMaxVoices;
   ...
   clap_process_status process(const clap_process_t *pr) {
      ...
      mGestureMeter.store(mEngine.gestureCounter(), std::memory_order_relaxed);
   ...
   uint32_t guiVoiceLimit() const override {
      return static_cast<uint32_t>(realValue(kParamMaxVoices));
   }

   uint32_t guiEventCounter() const override {
      return mGestureMeter.load(std::memory_order_relaxed);
   }
   ...
   static bool guiCreate(const clap_plugin_t *p, const char *api, bool isFloating) {
      ...
      plug->mGui = whooshpact::createGui(*plug);
      if (!plug->mGui)
         return false;
      plug->startGuiClock();
      return true;
   ...
   std::atomic<uint32_t> mGestureMeter{0};
...
const clap_plugin_t *factoryCreate(const clap_plugin_factory_t *, const clap_host_t *host,
   ...
   if (!pluginId || std::strcmp(pluginId, kPluginId) != 0)
      return nullptr;
   auto *plug = new WhooshPactPlugin(host);
...
bool whooshpactEntryInit(const char *) { return true; }
void whooshpactEntryDeinit() {}

const void *whooshpactEntryGetFactory(const char *factoryId) {
   if (!factoryId)
      return nullptr;
   if (std::strcmp(factoryId, CLAP_PLUGIN_FACTORY_ID) == 0)
      return &whooshpact::gPluginFactory;
   if (std::strcmp(factoryId, CLAP_PRESET_DISCOVERY_FACTORY_ID) == 0 ||
       std::strcmp(factoryId, CLAP_PRESET_DISCOVERY_FACTORY_ID_COMPAT) == 0)
      return whooshpact::presetDiscoveryFactory();
   return nullptr;
}
}
