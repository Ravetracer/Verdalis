// Opens WhooshPact's editor in a plain X11 window so the interface can be driven
...
//
//   ./whooshpact-guihost [plugin.clap] [preset.whooshpact] [seconds]
//
...
int main(int argc, char **argv) {
   const std::string pluginPath = argc > 1 ? argv[1] : "./WhooshPact.clap";
   ...
   host.name = "whooshpact-guihost";
   host.vendor = "WhooshPact";
   host.url = "";
   host.version = "1.0";
   ...
   wc.lpszClassName = L"WhooshPactGuiHost";
   RegisterClassExW(&wc);

   RECT wanted{0, 0, static_cast<LONG>(w), static_cast<LONG>(h)};
   AdjustWindowRect(&wanted, WS_OVERLAPPEDWINDOW, FALSE);
   HWND hostWindow = CreateWindowExW(0, L"WhooshPactGuiHost", L"WhooshPact", WS_OVERLAPPEDWINDOW,
