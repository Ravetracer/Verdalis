// Minimal CLAP host used to verify WhooshPact outside a DAW: it walks the preset
...
//
//   whooshpact-render --list
//   whooshpact-render --preset simple_whoosh --out whoosh.wav --seconds 8
//   whooshpact-render --all --outdir /tmp/whooshpact
//   whooshpact-render --selftest
...
void setupIndexer() {
   ...
   gIndexer.iface.name = "whooshpact-render";
   gIndexer.iface.vendor = "WhooshPact";
...
int main(int argc, char **argv) {
   std::string pluginPath = "./WhooshPact.clap";
...
int runSelfTest(const clap_plugin_entry_t *entry, double sampleRate) {
   ...
   if (state) {
      ...
      const std::string junk = "not a whooshpact state blob at all";
      ReadCtx rc2{&junk, 0};
   ...
   // preset quietly changes the sound it was saved from.
   {
      ...
      check(text.find("type = ") != std::string::npos &&
               text.find("type = 0") == std::string::npos,
      ...
      const std::string tmpdir =
         (std::filesystem::temp_directory_path() /
          ("whooshpact-selftest-" + std::to_string(
#if defined(_WIN32)
