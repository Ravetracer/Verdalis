// Minimal CLAP host used to verify InsectSwarm outside a DAW: it walks the preset
...
//
//   insectswarm-render --list
//   insectswarm-render --preset honeybee_field --out swarm.wav --seconds 8
//   insectswarm-render --all --outdir /tmp/swarm
//   insectswarm-render --selftest
...
void setupIndexer() {
   ...
   gIndexer.iface.name = "insectswarm-render";
   gIndexer.iface.vendor = "InsectSwarm";
...
int main(int argc, char **argv) {
   std::string pluginPath = "./InsectSwarm.clap";
   std::string presetSel, outPath = "river.wav", outDir = ".";
...
int runSelfTest(const clap_plugin_entry_t *entry, double sampleRate) {
   ...
   if (state) {
      ...
      const std::string junk = "not a insectswarm state blob at all";
      ReadCtx rc2{&junk, 0};
   ...
   // --- a fixed Random Seed has to mean the same river every time, whatever the
   ...
   // preset quietly changes the sound it was saved from.
   {
      ...
      // Checked by what follows the key rather than against one expected name,
      // so the test does not have to know which choice the fraction above lands
      // on -- an index would start with a digit, a name never does.
      {
         const size_t at = text.find("species = ");
         const bool byName =
            at != std::string::npos &&
            !std::isdigit(static_cast<unsigned char>(text[at + std::strlen("species = ")]));
         check(byName, "enum parameters are written by name, not by index");
      }
      ...
      const std::string path = userPresetPath("My River / 2 **");
      check(path.empty() || path.find("My_River_2.") != std::string::npos,
      ...
      const std::string tmpdir =
         (std::filesystem::temp_directory_path() /
          ("insectswarm-selftest-" + std::to_string(
#if defined(_WIN32)
