...
constexpr char kPluginId[] = "de.ravetracer.insectswarm";
constexpr char kPluginName[] = "InsectSwarm";
constexpr char kPluginVendor[] = "Ravetracer";
constexpr char kPluginVersion[] = "0.3.2";
...
#ifdef INSECTSWARM_CMAKE_VERSION
constexpr bool sameString(const char *a, const char *b) {
   return *a == *b && (*a == '\0' || sameString(a + 1, b + 1));
}
static_assert(sameString(kPluginVersion, INSECTSWARM_CMAKE_VERSION),
...
constexpr char kPluginDescription[] =
   "Fully synthetic insects. No samples: every wingbeat, every flyby and every cicada's "
   "tymbal is computed while it plays, from measurements of 67 field recordings.";

constexpr char kPresetExtension[] = "insectswarm";
constexpr char kProviderId[] = "de.ravetracer.insectswarm.preset-provider";
