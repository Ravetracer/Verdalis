...
#ifdef INSECTSWARM_WITH_GUI
class InsectSwarmPlugin final : public GuiDelegate {
#else
class InsectSwarmPlugin {
#endif
public:
   explicit InsectSwarmPlugin(const clap_host_t *host) : mHost(host) {
...
private:
   static InsectSwarmPlugin *self(const clap_plugin_t *p) {
      return static_cast<InsectSwarmPlugin *>(p->plugin_data);
   }
   ...
   static bool audioPortsGet(const clap_plugin_t *, uint32_t index, bool isInput,
      ...
      std::memset(info, 0, sizeof(*info));
      info->id = 0;
      std::snprintf(info->name, sizeof(info->name), "Water Out");
   ...
   static bool voiceInfoGet(const clap_plugin_t *, clap_voice_info_t *info) {
      info->voice_count = SwarmEngine::kMaxVoices;
      info->voice_capacity = SwarmEngine::kMaxVoices;
   ...
   clap_process_status process(const clap_process_t *pr) {
      ...
      // state directly.
      mEventMeter.store(mEngine.activeEventCount(), std::memory_order_relaxed);
      mFlybyMeter.store(mEngine.flybyCounter(), std::memory_order_relaxed);
   ...
   uint32_t guiVoiceCount() const override {
      return mEventMeter.load(std::memory_order_relaxed);
   }
   ...
   uint32_t guiEventCounter() const override {
      return mFlybyMeter.load(std::memory_order_relaxed);
   }
   ...
   static bool guiCreate(const clap_plugin_t *p, const char *api, bool isFloating) {
      ...
      plug->mGui = insectswarm::createGui(*plug);
      if (!plug->mGui)
         return false;
      plug->startGuiClock();
      return true;
   ...
   std::atomic<uint32_t> mEventMeter{0};
   std::atomic<uint32_t> mFlybyMeter{0};
...
const clap_plugin_t *factoryCreate(const clap_plugin_factory_t *, const clap_host_t *host,
   ...
   if (!pluginId || std::strcmp(pluginId, kPluginId) != 0)
      return nullptr;
   auto *plug = new InsectSwarmPlugin(host);
...
bool insectswarmEntryInit(const char *) { return true; }
void insectswarmEntryDeinit() {}

const void *insectswarmEntryGetFactory(const char *factoryId) {
   if (!factoryId)
      return nullptr;
   if (std::strcmp(factoryId, CLAP_PLUGIN_FACTORY_ID) == 0)
      return &insectswarm::gPluginFactory;
   if (std::strcmp(factoryId, CLAP_PRESET_DISCOVERY_FACTORY_ID) == 0 ||
       std::strcmp(factoryId, CLAP_PRESET_DISCOVERY_FACTORY_ID_COMPAT) == 0)
      return insectswarm::presetDiscoveryFactory();
   return nullptr;
}
}
