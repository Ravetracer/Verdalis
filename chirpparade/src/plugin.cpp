...
#ifdef CHIRPPARADE_WITH_GUI
class ChirpParadePlugin final : public GuiDelegate {
#else
class ChirpParadePlugin {
#endif
public:
   explicit ChirpParadePlugin(const clap_host_t *host) : mHost(host) {
...
private:
   static ChirpParadePlugin *self(const clap_plugin_t *p) {
      return static_cast<ChirpParadePlugin *>(p->plugin_data);
   }
   ...
   static bool audioPortsGet(const clap_plugin_t *, uint32_t index, bool isInput,
      ...
      std::memset(info, 0, sizeof(*info));
      info->id = 0;
      std::snprintf(info->name, sizeof(info->name), "Bird Out");
   ...
   static bool voiceInfoGet(const clap_plugin_t *, clap_voice_info_t *info) {
      info->voice_count = ChirpEngine::kMaxVoices;
      info->voice_capacity = ChirpEngine::kMaxVoices;
   ...
   clap_process_status process(const clap_process_t *pr) {
      ...
      // state directly.
      mChirpMeter.store(mEngine.activeChirpCount(), std::memory_order_relaxed);
      mSyllableMeter.store(mEngine.syllableCounter(), std::memory_order_relaxed);
   ...
   uint32_t guiVoiceCount() const override {
      return mChirpMeter.load(std::memory_order_relaxed);
   }
   ...
   // The header ornament writes a stroke on its sonogram for every syllable, so
   // it needs the count rather than only the load.
   uint32_t guiEventCounter() const override {
      return mSyllableMeter.load(std::memory_order_relaxed);
   }

   // Clicking the version label fires one free phrase: a bird, with no note
   // behind it. The window runs on the main thread and must not touch the
   ...
   static bool guiCreate(const clap_plugin_t *p, const char *api, bool isFloating) {
      ...
      plug->mGui = chirpparade::createGui(*plug);
      if (!plug->mGui)
         return false;
      plug->startGuiClock();
      return true;
   ...
   std::atomic<uint32_t> mChirpMeter{0};
   std::atomic<uint32_t> mSyllableMeter{0};
...
const clap_plugin_t *factoryCreate(const clap_plugin_factory_t *, const clap_host_t *host,
   ...
   if (!pluginId || std::strcmp(pluginId, kPluginId) != 0)
      return nullptr;
   auto *plug = new ChirpParadePlugin(host);
...
static const void *entryGetFactory(const char *factoryId) {
   ...
   if (std::strcmp(factoryId, CLAP_PLUGIN_FACTORY_ID) == 0)
      return &chirpparade::gPluginFactory;
