// Opens ChirpParade's editor in a plain X11 window so the interface can be driven
...
//
//   ./chirpparade-guihost [plugin.clap] [preset.chirpparade] [seconds]
//
...
int main(int argc, char **argv) {
   const std::string pluginPath = argc > 1 ? argv[1] : "./ChirpParade.clap";
   ...
   host.name = "chirpparade-guihost";
   host.vendor = "ChirpParade";
   host.url = "";
   host.version = "1.0";
   ...
   wc.lpszClassName = L"ChirpParadeGuiHost";
   RegisterClassExW(&wc);

   RECT wanted{0, 0, static_cast<LONG>(w), static_cast<LONG>(h)};
   AdjustWindowRect(&wanted, WS_OVERLAPPEDWINDOW, FALSE);
   HWND hostWindow = CreateWindowExW(0, L"ChirpParadeGuiHost", L"ChirpParade", WS_OVERLAPPEDWINDOW,
   ...
   XStoreName(dpy, win, "ChirpParade");
