// Minimal CLAP host used to verify ChirpParade outside a DAW: it walks the preset
...
//
//   chirpparade-render --list
//   chirpparade-render --preset dawn_chorus --out birds.wav --seconds 20
//   chirpparade-render --all --outdir /tmp/birds
//   chirpparade-render --selftest
//   chirpparade-render --defaults        > presets/new.chirpparade
...
void setupIndexer() {
   ...
   gIndexer.iface.name = "chirpparade-render";
   gIndexer.iface.vendor = "ChirpParade";
...
int main(int argc, char **argv) {
   std::string pluginPath = "./ChirpParade.clap";
   std::string presetSel, outPath = "birds.wav", outDir = ".";
   ...
   if (doDefaults) {
      using namespace chirpparade;
      const ParamDesc *table = paramTable();
      PresetData data;
      data.name = "Defaults";
      data.author = "ChirpParade";
...
int runSelfTest(const clap_plugin_entry_t *entry, double sampleRate) {
   ...
   if (state) {
      ...
      const std::string junk = "not a chirpparade state blob at all";
      ReadCtx rc2{&junk, 0};
   ...
   // --- a fixed Random Seed has to mean the same birds every time, whatever the
   ...
   // preset quietly changes the sound it was saved from.
   {
      ...
      check(text.find("species = ") != std::string::npos &&
               text.find("species = 0") == std::string::npos,
      ...
      const std::string path = userPresetPath("My Birds / 2 **");
      check(path.empty() || path.find("My_Birds_2.") != std::string::npos,
      ...
      const std::string tmpdir =
         (std::filesystem::temp_directory_path() /
          ("chirpparade-selftest-" + std::to_string(
#if defined(_WIN32)
