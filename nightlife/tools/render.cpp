// Minimal CLAP host used to verify NightLife outside a DAW: it walks the preset
...
//
//   nightlife-render --list
//   nightlife-render --preset wolf_valley --out night.wav --seconds 20
//   nightlife-render --all --outdir /tmp/night
//   nightlife-render --selftest
//   nightlife-render --defaults        > presets/new.nightlife
...
void setupIndexer() {
   ...
   gIndexer.iface.name = "nightlife-render";
   gIndexer.iface.vendor = "NightLife";
...
int main(int argc, char **argv) {
   std::string pluginPath = "./NightLife.clap";
   std::string presetSel, outPath = "night.wav", outDir = ".";
   ...
   if (doDefaults) {
      using namespace nightlife;
      const ParamDesc *table = paramTable();
      PresetData data;
      data.name = "Defaults";
      data.author = "NightLife";
...
int runSelfTest(const clap_plugin_entry_t *entry, double sampleRate) {
   ...
   if (state) {
      ...
      const std::string junk = "not a nightlife state blob at all";
      ReadCtx rc2{&junk, 0};
   ...
   // --- a fixed Random Seed has to mean the same night every time, whatever the
   ...
   // preset quietly changes the sound it was saved from.
   {
      ...
      check(text.find("caller = ") != std::string::npos &&
               text.find("caller = 0") == std::string::npos,
      ...
      const std::string path = userPresetPath("My Night / 2 **");
      check(path.empty() || path.find("My_Night_2.") != std::string::npos,
      ...
      const std::string tmpdir =
         (std::filesystem::temp_directory_path() /
          ("nightlife-selftest-" + std::to_string(
#if defined(_WIN32)
