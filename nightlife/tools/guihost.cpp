// Opens NightLife's editor in a plain X11 window so the interface can be driven
...
//
//   ./nightlife-guihost [plugin.clap] [preset.nightlife] [seconds]
//
...
int main(int argc, char **argv) {
   const std::string pluginPath = argc > 1 ? argv[1] : "./NightLife.clap";
   ...
   host.name = "nightlife-guihost";
   host.vendor = "NightLife";
   host.url = "";
   host.version = "1.0";
   ...
   wc.lpszClassName = L"NightLifeGuiHost";
   RegisterClassExW(&wc);

   RECT wanted{0, 0, static_cast<LONG>(w), static_cast<LONG>(h)};
   AdjustWindowRect(&wanted, WS_OVERLAPPEDWINDOW, FALSE);
   HWND hostWindow = CreateWindowExW(0, L"NightLifeGuiHost", L"NightLife", WS_OVERLAPPEDWINDOW,
