...
constexpr uint32_t kStateMagic = 0x50524843u; // 'CHRP' little-endian
constexpr uint32_t kStateVersion = 1;

} // namespace

#ifdef NIGHTLIFE_WITH_GUI
class NightLifePlugin final : public GuiDelegate {
#else
class NightLifePlugin {
#endif
public:
   explicit NightLifePlugin(const clap_host_t *host) : mHost(host) {
...
private:
   static NightLifePlugin *self(const clap_plugin_t *p) {
      return static_cast<NightLifePlugin *>(p->plugin_data);
   }
   ...
   static bool audioPortsGet(const clap_plugin_t *, uint32_t index, bool isInput,
      ...
      std::memset(info, 0, sizeof(*info));
      info->id = 0;
      std::snprintf(info->name, sizeof(info->name), "Night Out");
   ...
   static bool voiceInfoGet(const clap_plugin_t *, clap_voice_info_t *info) {
      info->voice_count = NightEngine::kMaxVoices;
      info->voice_capacity = NightEngine::kMaxVoices;
   ...
   clap_process_status process(const clap_process_t *pr) {
      ...
      // state directly.
      mCallMeter.store(mEngine.activeCallCount(), std::memory_order_relaxed);
      mCallCounterMeter.store(mEngine.callCounter(), std::memory_order_relaxed);
   ...
   uint32_t guiVoiceCount() const override {
      return mCallMeter.load(std::memory_order_relaxed);
   }
   ...
   // The header ornament draws a new hill for every call, so it needs the count
   // rather than only the load.
   uint32_t guiEventCounter() const override {
      return mCallCounterMeter.load(std::memory_order_relaxed);
   }

   // Clicking the version label fires one free phrase: an animal calling, with
   // no note behind it. The window runs on the main thread and must not touch the
   // engine, so the click is counted here and the audio thread consumes the
   // count on its next process(). request_process() is what makes it work with
   // no note held: the engine reports silence when nothing is sounding, so the
   // host may well have stopped calling process at all.
   ...
   static bool guiCreate(const clap_plugin_t *p, const char *api, bool isFloating) {
      ...
      plug->mGui = nightlife::createGui(*plug);
      if (!plug->mGui)
         return false;
      plug->startGuiClock();
      return true;
   ...
   std::atomic<uint32_t> mCallMeter{0};
   std::atomic<uint32_t> mCallCounterMeter{0};
...
const clap_plugin_t *factoryCreate(const clap_plugin_factory_t *, const clap_host_t *host,
   ...
   if (!pluginId || std::strcmp(pluginId, kPluginId) != 0)
      return nullptr;
   auto *plug = new NightLifePlugin(host);
...
bool nightlifeEntryInit(const char *) { return true; }
void nightlifeEntryDeinit() {}

const void *nightlifeEntryGetFactory(const char *factoryId) {
   if (!factoryId)
      return nullptr;
   if (std::strcmp(factoryId, CLAP_PLUGIN_FACTORY_ID) == 0)
      return &nightlife::gPluginFactory;
   if (std::strcmp(factoryId, CLAP_PRESET_DISCOVERY_FACTORY_ID) == 0 ||
       std::strcmp(factoryId, CLAP_PRESET_DISCOVERY_FACTORY_ID_COMPAT) == 0)
      return nightlife::presetDiscoveryFactory();
   return nullptr;
}
}
